...
class S3StorageAdapter implements StorageAdapter {
  ...
  Uri _buildObjectUri(String objectKey) {
    ...
    if (config.usePathStyle) {
      ...
      return Uri.parse('$scheme://$endpoint/${config.bucketName}/$cleanKey');
    } else {
      // Virtual-hosted-style: https://bucket.endpoint/key (AWS standard)
      return Uri.parse('$scheme://${config.bucketName}.$endpoint/$cleanKey');
    }
  }
  ...
  Future<bool> testConnection() async {
    try {
      ...
      final response = await http.head(uri, headers: headers).timeout(const Duration(seconds: 10));
  ...
  Future<List<String>> listRemoteFiles(String remoteDir) async {
    try {
      final uri = _buildObjectUri('?list-type=2&prefix=$remoteDir');
