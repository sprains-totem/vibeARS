...
7. **Known Implementation Constraints (verified by CI)**:
   - Recording formats: **WAV (PCM+RIFF)**, **AAC/M4A** (Android MediaCodec+MediaMuxer, iOS AVAudioFile), **MP3** (bundled LAME via `flutter_lame` 1.0.3 — WAV slices transcoded after recording stops; the `dart_lame` 1.0.3 API uses NAMED constructor args and async `encode(leftChannel:)/flush()/close()`), and **Opus (.ogg)** — Opus is implemented on **Android only** (system MediaCodec `audio/opus` + OGG muxer, API 29+); on iOS any non-m4a format falls back to WAV until Opus is wired up. AAC/MP3/Opus respect the configured bit rate.
